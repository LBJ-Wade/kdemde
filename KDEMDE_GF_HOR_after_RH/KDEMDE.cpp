...
int main(int argc, const char * argv[]) {
    ...
    //==========Parameters===================
    double c=atof(argv[2]);//0.5; //0.24;// c is defined as sqrt(2T_d/m_DM)
    double kTokRH1;
    double kTokRH1_min=atof(argv[1]), kTokRH1_max=2.1;//5.e-3, kTokRH1_max=1.0;
    double T_REH=5; //15
    double T_kdS=10; //5
    ...
    double a_REH=3000, a_fin=100*a_REH; //fixed only in the after case
    ...
    create_index_map();//computes and stores an index map from column to n,l

    
    Params P ={a_fin, c, kTokRH1, T_REH, T_kdS,m_DM, a_REH,kT};
    glob_Par=&P;
    ...
    {
        kTokRH1= kvals[i];
        kT=kTokRH1*pow((a_I/a_REH),0.5)*a_I;
        cout<<"kTilde="<<kT<<endl;
        P.kTokRH1=kTokRH1;
        P.kTilde=kT;
        ...
        cout<<"GammaT="<<GammaTilde(&P)<<" UpsiIni="<<UpsiPar(&InitialBkgDens, &P)<<" kTilde="<<glob_Par->kTilde<<endl;
