...
#pragma omp parallel for default(shared) schedule(static, 1) reduction(+:scalar)
#endif
...
#pragma omp parallel for default(shared) schedule(static, 1)
    ...
    //===================================================================================
    void ODE_solver_compute_Jacobian_Matrix_full(double z, double Dz, int neq, double *y,
                                            ...
                                            ODE_Solver_data &SD, int mess=0)
    {
        if(Jac.A.size()==0)
        {
            ODE_solver_compute_Jacobian_Matrix_full(z, Dz, neq, y, Jac, SD, mess);
            return;
        }
...
#pragma omp parallel for default(shared) schedule(static, 1)
                                        ...
                                        ODE_solver_Solution *Sn[6])
    {
        ...
        do{
            ...
            //for(int k=0; k<neq; k++) cout << k << " " << ODE_Solver_info.Sguessptr->dy[k] << endl;
            //wait_f_r();
            ...
            if(converged==0)
            {
                ...
                {
                    tolJac/=2.0;
                    cout <<"at xin="<< zin<<" Tightening Jac error setting at it-# " << Jac_loops << endl;
                }
