...
void fcn_k_eval(int *neq, double *t, double *y, double *f, int col)
{
    ...
    double c=glob_Par->c, kT=kTilde,TkdS=glob_Par->T_kdS, m_DM=glob_Par->m_DM;
    ...
    //============================================================
    if(abs(y[ithetaR])<1.0e-5 &&  abs(y[iPhi])<1.e-10) f[ideltaR]=f[ithetaR]=0.0;
    
    
    //============================================================
    // switch rhoPhi evolution when it becomes small
    //============================================================
    if(abs(y[iBkgP])<1.0e-20) f[iBkgP]=0.0;
...
void output_current_DMpert_k(ofstream &ofile, const ODE_solver_Solution &Sz)
{
    ...
    ofile << Sz.z << " "<< Sz.y[column[0][0]] << " ";
    ofile << 1.5*kTilde*Sz.y[column[0][1]]*(decoupled ? sqrt(2.0*TL/glob_Par->m_DM): glob_Par->c*sqrt(TL/glob_Par->T_kdS));
...
double setup_kevol(struct Background *bkgdens, void * parameters, struct Output_Files *pFiles, struct Growth_Function_File *pGFFile)
{
    ...
    if (decoupled and ((Nmax!=0 or Lmax!=1))){
        ...
        int nsteps=3000;//min(500, 100*int(P.a_fin));// 300;
        ...
        tols.abs[pExtInd->BkgP] = 1.0e-25;// the code is very sensitive to this value. Need to figure a way to 'automate' adjustment based on other params
        tols.abs[pExtInd->BkgR] = 1.0e-3;
        ...
        Sx.y[pExtInd->deltaR] = PhiIni+ 46.0/63.0*pow(kTilde,2)*PhiIni*a_I;
        Sx.y[pExtInd->thetaR] = -2.0/3.0*pow(kTilde, 2)*PhiIni*sqrt(a_I);
        Sx.y[pExtInd->deltaP] = 2*PhiIni+2.0/3.0*pow(kTilde, 2)*PhiIni*a_I;
        Sx.y[pExtInd->thetaP] = -2.0/3.0*pow(kTilde, 2)*PhiIni*sqrt(a_I);
        ...
        Sx.y[column[0][1]] = 2.0*Sx.y[pExtInd->thetaR]/(3.0*kTilde)*(decoupled? sqrt(P.m_DM*Inv_Ti/2.0): sqrt(P.T_kdS*Inv_Ti)/P.c);
