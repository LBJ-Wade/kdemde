...
int ICBkgRho_TDM(struct Background *bkg_ivals, void *parameters){
    ...
    bkg_ivals->TDM= (decoupled? TFromRhoT(rhorI, &par)*pow(a_I/astar, alpha-2.0):TFromRhoT(rhorI, &par)*RatioIni);//THIS INCLUDES CASE FOR PARTIAL COUPLED a_KD<1!!!!
...
void create_output_files(struct Output_Files *ptrFiles){
    ...
    double aRH=a_HOR*pow(kTokRH1,2.0);
    ...
    sprintf(pFT_Bkg, "./DATAFILES/Bkg_CPLD_Nmax%dLmax%dTRH%.0ETkds%0.EkTokRH1_%0.E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1);

    if (decoupled)
    {
        
        sprintf(pFT_Pert, "./DATAFILES/DcpldPert_Nmax%dLmax%dTRH%0.ETkds%0.EkTokRH1_%0.EmDM%.0E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1,mDM);
        sprintf(pFT_DM_Pert,"./DATAFILES/DcpldPertDM_Nmax%dLmax%dTRH%0.ETkds%0.EaRH%2.2EkTokRH1_%2.2EmDM%.0E.dat",Nmax,Lmax,T_RH,Tkds,aRH,kTokRH1,mDM);
        sprintf(pFT_FULL_DM_Pert,"./DATAFILES/DcpldPertDM_FULL_Nmax%dLmax%dTRH%0.ETkds%0.EkTokRH1_%0.EmDM%.0E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1,mDM);
    ...
    else
    {
        ...
        sprintf(pFT_Pert, "./DATAFILES/Pert_Nmax%dLmax%dTRH%0.ETkds%0.EkTokRH1_%0.Ec%.0E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1,c);
        sprintf(pFT_DM_Pert,"./DATAFILES/PertDM_Nmax%dLmax%dTRH%1.ETkds%0.EaRH%2.2EkTokRH1_%2.2Ec%2.2E.dat",Nmax,Lmax,T_RH,Tkds,aRH,kTokRH1,c);
        sprintf(pFT_FULL_DM_Pert,"./DATAFILES/PertDM_FULL_Nmax%dLmax%dTRH%.0ETkds%0.EkTokRH1_%0.Ec%.0E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1,c);
...
void create_output_file_growth_function(struct Growth_Function_File *ptrFile)
{
    ...
    if (decoupled)
    {
        sprintf(pGF_DM, "./DATAFILES/DcpldPert_GF_1_DM_Nmax%dLmax%dTRH%0.ETkds%0.EkTokRH1_%0.EmDM%.0E.dat",Nmax,Lmax,T_RH,Tkds,kTokRH1,mDM);
    }
    
    else
    {
        sprintf(pGF_DM,"./DATAFILES/PertDM_GF_1_Nmax%dLmax%dTRH%.0ETkds%0.Ec%.0E.dat",Nmax,Lmax,T_RH,Tkds,c);
    }
